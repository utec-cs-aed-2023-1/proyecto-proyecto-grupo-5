...
const unsigned int Sha256Hash::hash_keys[64] =
{
...
void Sha256Hash::compress(const unsigned char *message, unsigned int block_nb)
{
    register_32 w[64];
...
void Sha256Hash::adjust_digest(const unsigned char *text, unsigned int text_len)
{
...
void Sha256Hash::digest_final(unsigned char *digest)
{
    unsigned int block_nb;
    unsigned int pm_len;
    unsigned int len_b;
...
{
    unsigned char digest[Sha256Hash::PADD_SIZE];
    memset(digest, 0, Sha256Hash::PADD_SIZE);
    ...
    char buf[2*Sha256Hash::PADD_SIZE+1];
    buf[2*Sha256Hash::PADD_SIZE] = 0;
    for (int i = 0; i < (int) Sha256Hash::PADD_SIZE; i++) { sprintf(buf+i*2, "%02x", digest[i]); }
